...
<head>
<title>PCNLekki-New Comers Page</title>
    <meta charset="utf-8"> 
...
<style type="text/css">
table {margin: 0;}
table tr th, table tr td{font-size: 0.65rem; width:100%; overflow:auto;}
...
#newcomertable { overflow-x:auto;}
</style>
<script>
function getUsers(){
    $.ajax({
        type: 'POST',
        url: 'newComerAction.php', //editable: change to action file
...
function visitorAction(type,id){
    ...
    $.ajax({
        type: 'POST',
        url: 'newComerAction.php', //editable
        data: visitorData,
...
function editUser(id){
    $.ajax({
        type: 'POST',
        dataType:'JSON',
        url: 'newComerAction.php', //editable: change to action
        ...
        success:function(data){
			...
			$('#first_attendanceEdit').val(data.first_attendance);
			$('#worship_reasonEdit').val(data.worship_reason);
			$('#hostnameEdit').val(data.hostname);
			$('#needsEdit').val(data.needs);
			$('#locationEdit').val(data.location);
			$('#partnerEdit').val(data.partner);
            $('#editForm').slideDown();
        }
    });
}
...
<div class="mdl-layout mdl-js-layout mdl-layout--fixed-drawer
  ...
  <header class="demo-header mdl-layout__header mdl-color--grey-100 mdl-color-text--grey-600">
        <div class="mdl-layout__header-row">
          <span class="mdl-layout-title">New Comers</span>
        ...
        <main class="mdl-layout__content">
          <div class="page-content">
			  <?php
                include "../../includes/layout/newComerFragment.php";
              ?> 
          </div>
        </main>
      </div> 
